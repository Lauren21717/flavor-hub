...
def profile(username):
    ...
    along with their created recipes.
    """
...
def logout():
    ...
    """
    # Code copied from Mini Project | https://learn.codeinstitute.net/
...
def add_recipe():
    ...
    Handles recipe creation by collecting form data
    and inserting it into the MongoDB collection 'recipes'.
    Redirects to the index page upon successful addition.
    Displays a 500 error page if an issue occurs.
    """
    try:
        if request.method == "POST":
            ...
            recipe = {
                ...
                "preparation_step": [
                    step.strip() for step in preparation_step
                    ],
...
def admin_dashboard():
    ...
    if current_user and current_user["username"] == "admin":
        ...
        return render_template(
            "admin_dashboard.html", categories=categories, all_users=all_users)
    ...
    if current_user and current_user["username"] == "admin":
        ...
        if user_to_delete and user_to_delete["username"] != "admin":
            ...
            mongo.db.recipes.delete_many(
                {"created_by": user_to_delete["username"]})

            # Delete the user
            mongo.db.users.delete_one({"_id": ObjectId(user_id)})

            flash("""User {user_to_delete['username']}
            and all their recipes have been deleted""")
        else:
...
def forbidden(e):
    ...
    when the user is not authorized to access the resource.
    """
